...
var CGrid = function (_React$PureComponent) {
	_inherits(CGrid, _React$PureComponent);
	...
	function CGrid(props, context) {
		...
		_this.componentDidUpdate = function (nextProps) {
			if (nextProps.columns && _this.props.columns) {
				var hasDiff = nextProps.columns.length != _this.props.columns.length;

				if (!hasDiff) for (var i = 0; i < nextProps.columns.length; i++) {
					if (nextProps.columns[i].label !== _this.props.columns[i].label) {
						hasDiff = true;break;
					}
				}

				if (hasDiff) _this.initGridSize();
			}
			...
			if (indexs.length > 0) {
				...
				if (k > 0) {
					...
					indexs.forEach(function (index, i) {
						if (i === indexs.length - 1) colWidths[index] = k - (indexs.length - 1) * l;else colWidths[index] = l;
					});
				} else indexs.forEach(function (index, i) {
	...
	_createClass(CGrid, [{
		...
		value: function render() {
			...
			var style = { height: '' + (pagination ? 'calc(100% - 50px)' : '100%') };
